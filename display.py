...
def draw_board(
    ...
    if border_params != None:
        ...
        for x in range(int(board.get_width()/neighbourhood_size)):
            drawing.line([(x * neighbourhood_size * CELL_WIDTH, 0),(x * neighbourhood_size * CELL_WIDTH, img_height)], fill = border_colour, width=1)
        for y in range(int(board.get_height()/neighbourhood_size)):
            drawing.line([(0, y * neighbourhood_size * CELL_HEIGHT), (img_width, y * neighbourhood_size * CELL_HEIGHT)], fill = border_colour, width=1)
