...
class Board1D(Board):
    ...
    def __getitem__(self, xy: Coordinate) -> int:
        (x, y) = xy
        if y == 0:
            return self._data[x]
        else:
    ...
    def update(self) -> None:
        ...
        def try_out_spots(x: int) -> None:
            ...
            lowest_spot: int = -1 if self._MAX_TRAVEL_DISTANCE is None or x - self._MAX_TRAVEL_DISTANCE < -1 else x - self._MAX_TRAVEL_DISTANCE
            highest_spot: int = len(self._data) if self._MAX_TRAVEL_DISTANCE is None  or x + self._MAX_TRAVEL_DISTANCE > len(self._data) else x + self._MAX_TRAVEL_DISTANCE

            current_spot: int = x
            nearby_spots = interleave(
                range(x - 1, lowest_spot , -1),
                range(x + 1, highest_spot, +1)
            )
